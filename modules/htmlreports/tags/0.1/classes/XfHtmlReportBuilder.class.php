...
class XfHtmlReportBuilder {
		...
		if ( $numLevels > 0 ){
			...
			for ($i=0; $i<$numLevels; $i++){
				...
				if ( isset($sectionFooterEls[$j]) ){
					$groupFields[$i] = array_merge($groupFields[$j], self::getFields($sectionFooterEls[$j]));
					$summaryFields[$i] = array_merge($summaryFields[$j], self::getSummaryFields($sectionFooterEls[$j]));
	...
	public static function extractRecordsFromGroupData(&$groupData){
		...
		if ( isset($groupData['records']) ){
			...
			foreach ($groupdData['sections'] as $key=>$section){
	 ...
	 */
	public function fillReport($root, $record, $el, $basePath=''){
	...
	function _replace_fields($matches){
		...
		if ( count($parts)>1 and $this->context->rrec ){
			...
			if ( isset($this->context->rrec) and $this->context->rrec->_relationshipName == $rel ){
				return $this->context->rrec->htmlValue($fld);
			} else {
	...
	function _replace_summary_fields($matches){
		...
		if ( isset($this->context->root->pouch['summaries'][$matches[1]]) ){
			...
			$this->context->root->setValue($fld, $val);
			$out = $this->context->root->htmlValue($fld);
			$this->context->root->setValue($fld, $old);
			return $out;
		}
