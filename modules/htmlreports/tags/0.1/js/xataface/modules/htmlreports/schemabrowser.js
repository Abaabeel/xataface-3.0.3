...
//require-css <xataface/modules/htmlreports/schemabrowser.css>
...
(function(){
	...
	var htmlreports = window.xataface.modules.htmlreports;
	htmlreports.SchemaBrowser = SchemaBrowser;
	...
	function initTreeEvents(sb, el){
		...
		$('li[xf-htmlreports-macro] a').click(function(event){
			var a = this;
			sb.trigger('fieldClicked', {
				sourceEvent: event,
				sourceElement: a,
				sourceBrowser: sb,
				fieldName: $(a).parent('li').attr('xf-htmlreports-fieldname'),
				macro: $(a).parent('li').attr('xf-htmlreports-macro')
			});
			return false;
		});
	}
	...
	function initTreePreview(sb, el){
		...
		$('li[xf-htmlreports-macro]', el).append('<span class="xf-preview">...</span>');
	...
	function refreshTreePreview(sb, el){
		...
		var q = {
			'-table': sb.query['-table'],
			'-action': 'htmlreports_schemabrowser_preview_row',
		...
		$.get(url, q, function(res){
			try {
				if ( res.code == 200 ){
					...
					$('li[xf-htmlreports-macro]', el).each(function(){
						var macro = $(this).attr('xf-htmlreports-macro');
	...
	SchemaBrowser.prototype = {
		...
		template:  @@(xataface/modules/htmlreports/schemabrowser/template.html),
		query: null,
		el: null,
		toolbar: null
	};
