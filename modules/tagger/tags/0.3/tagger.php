	...
	function afterSave($params){
		...
		if ( @$record->pouch['tagger__fields'] ){
			foreach ($record->pouch['tagger__fields'] as $f){
				...
				//$tremoves = array(); //array_diff($texistingLabels, $tval);
				//$tadds = array(); 	//array_diff($tval, $texistingLabels);
				
				
				
				foreach ($tremoves as $tid){
					$trec = $labelMap[$tid];
					$res = $io->removeRelatedRecord($trec, false, $secure);
				...
				foreach ($tadds as $tid){
					...
					$res = $io->addExistingRelatedRecord($trec, $secure);
					if ( PEAR::isError($res) ) return $res;
					unset($drec, $trec);
				}
